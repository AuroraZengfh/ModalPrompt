...
class Continual_LLAVA(LlavaLlamaForCausalLM):
    ...
    def set_comtinual_eval(self, tokenizer, clip_tokenizer, prefix_len, cur_task, tasks):
        '''
        set continual attributes for evaluation
        '''
        self.set_comtinual_prompts_tokenizer(tokenizer, prefix_len, tasks)
        self.set_clip_tokenizer(clip_tokenizer)
        self.set_proto_input_ids()
        self.cur_task = cur_task

    def set_comtinual_prompts_tokenizer(self, tokenizer, prefix_len =10, tasks = None, same_prompt=None):
        # adding special prefix tokens (CHANGE for new tasks)
        self.tokenizer = tokenizer
        self.prefix_len = prefix_len
        self.tasks = tasks
        self.num_tasks = len(self.tasks)        
        self.mapping_list = {'1':'scienceqa','2':'textvqa','3':'imagenet','4':'gqa','5':'vizwiz','6':'grounding','7':'vqav2','8':'ocrvqa'}
