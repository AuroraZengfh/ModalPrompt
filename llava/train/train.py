...
class LazySupervisedDataset(Dataset):
    ...
    def modality_lengths(self):
        length_list = []
        for sample in self.list_data_dict:
            cur_len = sum(len(conv['value'].split()) for conv in sample['conversations']) + min(self.cur_task, self.transfer_num) * self.prefix_len
...
def train():
    ...
    if model_args.vision_tower is not None:
        model = Continual_LLAVA.from_pretrained(
    ...
    if training_args.pt_enable:
        ...
        tokenizer = model.set_comtinual_prompts_tokenizer(tokenizer = tokenizer, prefix_len = model_args.prefix_len, tasks = ['scienceqa','textvqa','imagenet','gqa','vizwiz','grounding','vqav2','ocrvqa'])      
        ...
        model.set_cur_task(model_args.cur_task)
        model.set_clip_tokenizer(clip_tokenizer)
        model.set_proto_input_ids()
        
        # # only embedding and corresponding promptt ransform is trained
        # for name,param in model.named_parameters():
        #     if param.requires_grad == True:print(name)
        # reset model vocab_size as they are not changed in lm_head
        model.reset_vocab_size()
        model.training = True
        print('Setting forward mode to training.')

        # ensure model.model.embed_tokens.shape [32000 + prefix_len * num_task, 0496] requires_grad=True
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.pt_enable:
            ...
            #     if param.requires_grad == True:
            #         print(name)
            # for name,param in text_tower.named_parameters():
            #     if param.requires_grad == True:
            #         print(name)
    ...
    # set original token token embeds 
    model.set_origin_embed_tokens()
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args,
                                              prefix_len=model_args.prefix_len,
                                              cur_task=model_args.cur_task,
                                              transfer_num=model_args.transfer_num)
    ...
    elif training_args.pt_enable:
        model.set_previou_transform_for_save(model_args.cur_task)
        non_lora_state_dict = get_peft_state_non_lora_maybe_zero_3(
            model.named_parameters()
        )
        if training_args.local_rank == 0 or training_args.local_rank == -1:
